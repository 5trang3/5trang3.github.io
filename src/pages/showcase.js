...
import ShowcaseItem from '../components/ShowcaseItem.js'
...
export default () => {
  ...
  const showcaseItems = showcaseData.map((item) => <ShowcaseItem title={ item.title } img={ item.image } content={ item.content } chips={ item.chips } githubLink={ item.github } link={ item.link }/>)
